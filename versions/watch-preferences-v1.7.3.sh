...
#   $7 = ONLY_CMDS (true/false) — afficher uniquement les commandes defaults (défaut false)
...
log_line() {
  ...
  if [ "${ONLY_CMDS:-false}" = "true" ]; then
    ...
    if [ "${WATCH_ALL:-false}" = "true" ]; then
      echo "$out" | tee -a "$LOGFILE_USER" | tee -a "$LOGFILE_SYSTEM" >/dev/null
    else
      ...
      if [ -n "$kv" ]; then
        ...
        case "$dline" in
          +*)
            ...
            dom="${base%.plist}"
            hostflag=""
            if printf '%s' "$path" | grep -q "/ByHost/"; then
            ...
            if printf '%s' "$trimmed" | /usr/bin/grep -Eq '^".*"$'; then
              ...
              type_val=$(printf '%s' "$trimmed" | tr '[:lower:]' '[:upper:]')
