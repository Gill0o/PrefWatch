    ...
    while IFS= read -r dline; do
      ...
      if [ -n "$kv" ]; then
        ...
        if [ "$_has_array_additions" = "true" ] && [[ "$keyname" != *":"* ]]; then
          ...
          if [[ "$keyname" == *" "* ]] || [[ "$keyname" =~ ^(InputSourceKind|KeyboardLayout|tile-data|file-data|file-label|bundle-identifier|_CFURLString|_CFURLStringType).*$ ]]; then
            continue
          fi
        fi
