...
# Cache pour optimiser les vérifications répétées d'exclusion
...
is_excluded_domain() {
  ...
  # Calcul et mise en cache du résultat
  local p result=1
...
# Optimisation: Détection du binaire date au démarrage
HAVE_BIN_DATE="false"
[ -x /bin/date ] && HAVE_BIN_DATE="true"

# Fonction helper pour timestamp optimisée
get_timestamp() {
...
# Fonction helper pour déterminer le chemin plist depuis un domaine
get_plist_path() {
  local domain="$1"
  if [[ "$domain" =~ ^/ ]]; then
    ...
    # Domaine relatif → chemin utilisateur
