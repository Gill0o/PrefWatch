...
convert_to_plistbuddy() {
  ...
  # Calculate actual index by counting current array elements using Python
  local actual_index=0
  if [ -n "$PYTHON3_BIN" ] && [ -f "$plist_path" ]; then
    local json_data array_length
    json_data=$("${RUN_AS_USER[@]}" /usr/bin/defaults export "$domain" - 2>/dev/null | /usr/bin/plutil -convert json -o - - 2>/dev/null) || json_data=""
    if [ -n "$json_data" ]; then
      array_length=$(printf '%s' "$json_data" | "$PYTHON3_BIN" -c "import json, sys; data=json.load(sys.stdin); print(len(data.get('$key', [])) if isinstance(data.get('$key'), list) else 0)" 2>/dev/null) || array_length="0"
      actual_index="$array_length"
    fi
  fi
  ...
  while [ -n "$parsing" ]; do
    # Force disable xtrace in this critical section to prevent debug output leaking
    {
      set +x +v 2>/dev/null
      ...
      # Use actual_index instead of $INDEX placeholder
      pb_cmds+=("/usr/libexec/PlistBuddy -c 'Add :${key}:${actual_index}:${dict_key} ${value_type} ${dict_value}' \"${plist_path}\"")
