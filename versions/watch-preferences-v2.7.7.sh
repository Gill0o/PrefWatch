...
convert_to_plistbuddy() {
  ...
  # Calculate actual index by counting current array elements
  local actual_index=0
  if [ -f "$plist_path" ]; then
    # Try to get array length from plist
    local array_length
    array_length=$("${RUN_AS_USER[@]}" /usr/bin/defaults read "$domain" "$key" 2>/dev/null | /usr/bin/grep -c "^    " || echo "0")
    # If array exists, new element will be at index = current length
