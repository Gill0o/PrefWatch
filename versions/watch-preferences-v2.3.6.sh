...
#   $8 = EXCLUDE_DOMAINS — exclut la surveillance de certains domaines (glob)
#        Exemple: ContextStoreAgent*,com.jamf*,com.jamfsoftware*,com.launchdarkly*,com.adobe.*
#        Note: ces domaines ne seront ni surveillés ni loggés
...
EXCLUDE_DOMAINS_RAW="${EXCLUDE_DOMAINS:-${8:-ContextStoreAgent*,com.bjango.istatmenus.status,com.apple.systempreferences,com.apple.systemsettings.extensions,com.apple.systempreferencescom.apple.CrashReporter,com.apple.CloudKit,com.apple.DuetExpertCenter.AppPredictionExpert,com.jamf*,com.jamfsoftware*,com.launchdarkly*,com.apple.loginwindow,com.apple.Console,com.apple.knowledge-agent,com.apple.spaces,com.apple.networkextension,com.apple.xpc.activity2}}"
...
# Extrait le domaine d'une commande defaults write
...
convert_to_plistbuddy() { # convert_to_plistbuddy <defaults_command>
  ...
  # Génère un tableau de commandes PlistBuddy
  local -a pb_cmds

  # Parse les paires clé-valeur avec sed (compatible zsh)
  local parsing="$inside"
  while [ -n "$parsing" ]; do
    # Extrait la prochaine paire "key" = value; (sed -nE avec /p ne retourne qu'une ligne)
  ...
  # Retourne les commandes séparées par des newlines
...
convert_delete_to_plistbuddy() { # convert_delete_to_plistbuddy <defaults_delete_command>
  local cmd="$1"

  # Extrait le domaine et le target du defaults delete (gère -currentHost et autres flags)
  # Format: defaults [-currentHost] delete <domain> "<target>"
  local domain target

  # Extrait le domaine (premier arg après delete, en ignorant les flags)
  domain=$(printf '%s' "$cmd" | /usr/bin/sed -nE 's/.*defaults([[:space:]]+-[^[:space:]]+)*[[:space:]]+delete[[:space:]]+([^[:space:]]+).*/\2/p')

  # Extrait le target (peut être entre guillemets ou non)
  ...
  # Détecte si c'est une suppression dans un tableau (présence de :clé:index)
  # Exemple: ":AppleEnabledInputSources:2" ou ":persistent-apps:0"
  local is_array_deletion=false
  if printf '%s' "$target" | /usr/bin/grep -Eq ':[^:]+:[0-9]+$'; then
    is_array_deletion=true
  fi

  # Génère la commande PlistBuddy avec avertissement pour les tableaux
  if [ "$is_array_deletion" = "true" ]; then
    # Suppression dans un tableau - ajouter un commentaire d'avertissement
    printf '# ATTENTION: Suppression dans un tableau - les index changent après chaque suppression\n'
    printf '# Pour suppressions multiples: exécuter de l'\''index le plus GRAND vers le plus PETIT\n'
  fi
...
log_line() {
  ...
  if [ "${ONLY_CMDS:-false}" = "true" ]; then
    ...
    # Garde-fou global: supprime les commandes pour domaines exclus
  ...
  local line="[$ts] $msg"
  # Garde-fou global: si une ligne contient une commande defaults write pour domaine exclu, ne pas logguer
  ...
  while IFS=$'\t' read -r base idx keylist cmd; do
    ...
    if [ -n "$cmd" ]; then
      ...
      if is_noisy_command "$cmd"; then
        ...
        # Génère et log les alternatives PlistBuddy (une commande par ligne)
...
emit_array_deletions() { # emit_array_deletions <kind> <domain> <prev_json> <curr_json>
  local kind="$1" dom="$2" prev_json="$3" curr_json="$4"
  [ -n "$PYTHON3_BIN" ] || return 0
  [ -s "$curr_json" ] || return 0
  [ -s "$prev_json" ] || return 0
...
def diff_deletions(prev_obj, curr_obj, path):
    """Trouve les éléments supprimés en comparant prev avec curr"""
        ...
        for prev_idx, prev_item in enumerate(prev_obj):
            ...
            for curr_idx, curr_item in enumerate(curr_list):
                if not used[curr_idx] and prev_item == curr_item:
                    ...
                    break
            if not matched:
                # Cet élément était dans prev mais n'est plus dans curr → suppression
...
# Format de sortie: array_name\tindex\tkeys_list
...
PY
  ...
  # Pour chaque suppression détectée, générer une commande defaults delete
  while IFS=$'\t' read -r base idx keylist; do
    [ -n "$base" ] || continue

    # Construit la commande delete avec l'index
    ...
    if is_noisy_command "$delete_cmd"; then
      ...
      # Log la commande defaults delete
      case "$kind" in
      ...
      # Génère et log l'alternative PlistBuddy pour delete
      local pb_delete
      if pb_delete=$(convert_delete_to_plistbuddy "$delete_cmd" 2>/dev/null); then
        while IFS= read -r pb_line; do
          [ -n "$pb_line" ] || continue
          # Tous les commentaires et commandes ont le préfixe Cmd: pour passer le filtre ONLY_CMDS
          case "$kind" in
            USER) log_user "Cmd: $pb_line" ;;
            SYSTEM) log_system "Cmd: $pb_line" ;;
            DOMAIN) log_line "Cmd: $pb_line" ;;
...
show_plist_diff() { # show_plist_diff <USER|SYSTEM> <plist_path> [silent]
  ...
  if [ "$silent" != "true" ] && [ -n "$PYTHON3_BIN" ] && [ -s "$prev_json" ] && [ -s "$curr_json" ]; then
    # Détecte les ajouts d'éléments de tableaux
    ...
    # Détecte les suppressions d'éléments de tableaux
      ...
      if [ -n "$kv" ]; then
        ...
        case "$dline" in
          +*)
            ...
            # Filtre: ne log pas les commandes pour domaines exclus ou bruyantes
            local _cmd_dom
          ...
          -*)
            ...
            if is_noisy_command "$delete_cmd"; then
              ...
              if pb_delete=$(convert_delete_to_plistbuddy "$delete_cmd" 2>/dev/null); then
                while IFS= read -r pb_line; do
                  [ -n "$pb_line" ] || continue
                  # Les commentaires sont loggés sans préfixe Cmd: (seront filtrés en mode ONLY_CMDS)
                  # Les commandes sont loggées avec Cmd: (passeront le filtre ONLY_CMDS)
                  if [[ "$pb_line" == "#"* ]]; then
                    log_user "$pb_line"
                  else
