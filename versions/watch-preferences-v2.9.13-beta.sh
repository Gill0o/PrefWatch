  ...
  case "$domain" in
    ...
    com.apple.dock)
      case "$keyname" in
        ...
        GUID|dock-extra|tile-type|is-beta|_CFURLStringType|file-type|file-mod-date|parent-mod-date|book)
          return 0 ;;
...
for prefix, index, item in results:
    ...
    # Output PlistBuddy commands directly (handles nested dicts)
...
PY
  ...
  # Resolve plist path for PlistBuddy commands
  ...
  while IFS=$'\t' read -r col1 col2 rest; do
    ...
    # Handle PlistBuddy command lines (generated by Python for nested dicts)
    if [ "$col1" = "PBCMD" ]; then
      [ -n "$_pb_plist_path" ] || continue
      # Filter: extract leaf key from "Add :path:key type value"
      local _pb_path="${col2#Add :}"
      _pb_path="${_pb_path%% *}"
      local _pb_leaf="${_pb_path##*:}"
      # Skip noisy keys and binary data placeholders
      is_noisy_key "$dom" "$_pb_leaf" && continue
      [[ "$col2" == *"'<data:"* ]] && continue
      local pb_full="/usr/libexec/PlistBuddy -c '${col2}' \"${_pb_plist_path}\""
      case "$kind" in
        USER) log_user "Cmd: $pb_full" ;;
        SYSTEM) log_system "Cmd: $pb_full" ;;
        DOMAIN) log_line "Cmd: $pb_full" ;;
        *) log_line "Cmd: $pb_full" ;;
      esac
      continue
    fi

    # Metadata line: base\tidx\tkeys (for _skip_keys)
    outputs+=("${col1}\t${col2}\t${rest}")
  done <<< "$py_output"
