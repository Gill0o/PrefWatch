...
- The print-preset `# NOTE:` was keyed on a key no machine has, so it never fired. It now says the change needs a logout, and that neither the queue name nor a built-in preset name travels.
