...
- Array deletions are emitted highest-index-first so a batch of `Delete` commands is safe to run in the order shown — they were emitted lowest-first, and running them top-to-bottom (or copy-pasted into a script) deleted the wrong elements because each deletion shifts higher indexes down. WARNING text reworded to match.
