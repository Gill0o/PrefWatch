...
- Widened the default hot-domain set from `com.apple.finder,.GlobalPreferences` to also include `com.apple.dock`, `com.apple.controlcenter`, `com.apple.WindowManager`, `com.apple.systemsettings`. These common interactive panels are otherwise "cold" — cfprefsd can buffer their writes ~10s before fs_usage fires (e.g. adding/removing a Dock icon took ~10s to surface). Hot domains are flushed every 0.5s + re-diffed every ~2s, cutting that to ~1-2s. Override with `--hot-domains`.
