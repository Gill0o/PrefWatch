...
### Fix
- Responsiveness: the default hot-domain set now covers the common System Settings panels (Finder, Dock, Control Center, Window Manager, keyboard/trackpad/mouse, Accessibility, Spotlight, clock, screensaver, Software Update, …). Hot domains are flushed every 0.5s so their changes surface in ~1-2s instead of ~10s (cfprefsd buffers cold-domain writes). Broadening the set to ~20 domains is only affordable because the per-domain hot cost is just one parallel `defaults read`: the whole flush takes ≈0.15s/cycle, so display latency is unaffected and the only cost is fork volume. Use `--hot-domains` to override.
- Flush-loop freeze: a `defaults read` that hangs cfprefsd no longer freezes the poll loop for up to 4s — the straggler watchdog now fires at 1s/1.5s (was 3s/4s), capping any freeze at ~1.5s. The active-domain flush issues a single bare read per domain (the `-currentHost` variant is added only by `show_plist_diff`/`fs_watch` for actual `/ByHost/` plists) to keep the per-cycle fork count — and thus the hang surface — low. `com.apple.wallpaper` is excluded from the hot set because its real config lives in an internal Store (`~/Library/Application Support/com.apple.wallpaper/`), not the `com.apple.wallpaper` plist — so flushing it surfaces nothing. A full disk audit of every preference domain confirmed three more dead hot entries, now removed: `com.apple.sound` (not a real domain — sound prefs are `com.apple.sound.beep.*` in `.GlobalPreferences`, already hot), `com.apple.systemsettings` (no readable keys, window state only), `com.apple.touchbar` (absent on non-Touch-Bar Macs).
...
- `com.apple.loginwindow` is no longer domain-excluded — the audit found it was hiding real admin policies (`GuestEnabled`, `HideUserAvatarAndName`, `LoginwindowText`, `autoLoginUser`, `AdminHostInfo`, set in the system file). Its per-session churn (`lastUser`/`lastUserName`/`RecentUsers`/`AccountInfo`/`OptimizerPreviousBuild`, plus user-file `TALLogout*`/`MiniBuddy*`/`oneTimeSSMigrationComplete`) is filtered key-by-key instead; window frames were already global noise.
...
- Key filters: `CKPerBootTasks`, `DDMPersisted*`, `*-analytics-stamp` (all global — the last covers dock/screencapture/systemuiserver), `*_frame` (global — window/inspector geometry, e.g. SketchUp LayOut `*Inspector_frame`/`*Palette_frame`); Bartender `TerminationReasons`; AudioMIDISetup `audioDevice.selected`; iStat menubar `Updates`/`Status`; cloud.quota `_ICQ*`; AssetCache cache-size keys; ARDAgent `ARDAdmin_AppStoreURL`; RemoteDesktop `RSAKeySize`/`DOCAllowRemoteConnections`.
