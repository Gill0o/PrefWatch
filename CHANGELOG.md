...
- Pre-seed `HOT_DOMAINS` (`com.apple.dock`, `com.apple.finder`, `.GlobalPreferences`) into the active-domains registry at startup and auto-refresh them each poll cycle so they never expire. Eliminates the fs_usage→poll round-trip latency on the first change to these frequently-admin-touched domains. Override via `--hot-domains <list>` CLI flag or Jamf `$10` parameter (`NONE` disables).
